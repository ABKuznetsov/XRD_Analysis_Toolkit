...
$showcaseModule = Join-Path $appRoot "toolkit\first_run_showcase.ps1"
$runtimeSetupModule = Join-Path $appRoot "toolkit\sci_runtime_setup_ui.ps1"
...
try {
    ...
    while (-not $runtimeReady) {
        ...
        try {
            $showcase = Initialize-FirstRunShowcase -Owner $script:Form -AssetRoot (Join-Path $appRoot "toolkit\showcase") -Version "1.5.0" -Mode "Installing"
    ...
    if (-not (Test-ShowcaseSeen "1.5.0")) {
        $showcase = Initialize-FirstRunShowcase -Owner $script:Form -AssetRoot (Join-Path $appRoot "toolkit\showcase") -Version "1.5.0" -Mode "Ready"
        $script:Form.Hide()
