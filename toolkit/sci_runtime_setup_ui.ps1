function Show-RuntimeConsent {
    ...
    $dialog.Text = "XRD Phase Finder: scientific environment"
    ...
    $title.Text = "The scientific environment needs setup"
    ...
    $subtitle.Text = "XRD Phase Finder tested Python and its packages by running them."
    ...
    $label.Text = "Missing or not working:"
    ...
    $destination.Text = "Python and its packages will be updated or completed in:`r`n$EnvironmentPath"
    ...
    $install.Text = "Update or complete installation"
...
function Get-RuntimeSetupProgress {
    ...
    if (-not (Test-Path -LiteralPath $LogPath)) {
        return [pscustomobject]@{ Text = "Preparing installation"; Current = 0; Total = 0 }
    }
    try { $lines = @(Get-Content -LiteralPath $LogPath -Tail 80 -ErrorAction Stop) } catch {
        return [pscustomobject]@{ Text = "Preparing installation"; Current = 0; Total = 0 }
    }
    $joined = $lines -join "`n"
    $text = "Preparing the scientific environment"
    if ($joined -match "Downloading Python") { $text = "Downloading Python 3.11" }
    elseif ($joined -match "Installing Python") { $text = "Installing Python 3.11" }
    elseif ($joined -match "Creating venv") { $text = "Creating the Sci environment" }
    elseif ($joined -match "Upgrading pip") { $text = "Updating the pip package installer" }
    elseif ($joined -match "Installing package:\s*([^`r`n]+)") { $text = "Installing package: " + $Matches[1].Trim() }
    elseif ($joined -match "Collecting\s+([^\s`r`n]+)") { $text = "Downloading package: " + $Matches[1].Trim() }
    elseif ($joined -match "Runtime self-test") { $text = "Testing Python and scientific packages" }
    $current = 0
    $total = 0
...
function Invoke-VisibleSciRuntimeRepair {
    ...
    if ($process.ExitCode -ne 0) {
        ...
        throw "Installation exited with code $($process.ExitCode).`r`n`r`n$tail"
    }
}
...
function Show-RuntimeSetupFailure {
    ...
    $dialog = New-Object System.Windows.Forms.Form
    $dialog.Text = "XRD Phase Finder: installation incomplete"
    $dialog.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen
    $dialog.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::FixedDialog
    $dialog.MaximizeBox = $false
    $dialog.MinimizeBox = $false
    ...
    $title.Text = "The scientific environment could not be prepared"
